  ...
  processTiles() {
    for (let i = 0;i<this.tiles.length;i++) {
      for (let j = 0;j<this.tiles[i].length;j++) {
        this.tiles[i][j].func(this.tiles[i][j].position.x*100 - this.player.position.x,this.tiles[i][j].position.y*100-this.player.position.y)
      }
    }
  }
  logicFrame(inputPacket) {
    ...
    if (this.iterations ==0) {
      for (let i = 0;i<100;i++) {
        for (let j = 0;j<100;j++) {
          ...
          if (i == 0 || j == 0) {
            this.createTile(i,j,true,(x,y) => {this.drawController.newRect(0,x+600,y+337.5,100,100,[0,0,0])})
          }
        }
      }
    }
    ...
    this.drawController.newCircle(0,600,337.5,25,[255,127,255]) // player
    this.drawController.newCircle(0,600+20*Math.cos(this.player.rotation),337.5+20*Math.sin(this.player.rotation),5,[171,127,171]) // player eye
    this.drawController.newRect(0,-400- this.player.position.x,336.5- this.player.position.y,2000,2,[0,0,0]) // axis
    this.drawController.newRect(0,599- this.player.position.x,-662.5- this.player.position.y,2,2000,[0,0,0]) // axis
    this.processTiles()
  }
};
